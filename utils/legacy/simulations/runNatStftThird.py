...
with tf.variable_scope("Encoder"):
    filter_widths = [(7, 89), (4, 43), (2, 11), (2, 5), (2, 3)]
    input_channels = [2, 32, 128, 512, 128]
    output_channels = [32, 128, 512, 128, 64]
    strides = [[1, 1, 2, 1], [1, 2, 2, 1], [1, 2, 2, 1], [1, 2, 2, 1], [1, 2, 2, 1]]
...
# aModel.addReshape((batch_size, 3072))
# aModel.addFullyConnectedLayer(3072, 2827, 'Fully')
# aModel.addRelu()
# aModel.addReshape((batch_size, 11, 257, 1))

with tf.variable_scope("Decoder"):
    filter_widths = [(1, 3), (2, 3), (3, 4)]
    input_channels = [64, 256, 512]
    output_channels = [256, 512, 257]
    ...
    aModel.addDeconvLayer(filter_shape=(2, 111), input_channels=144, output_channels=11, stride=(1, 2, 1, 1),
                          name='first_deconv_after_reshape')
    aModel.addReshape((batch_size, 11, 257, 16))

    aModel.addDeconvLayerWithoutNonLin(filter_shape=(3, 11), input_channels=16, output_channels=2,
...
aContextEncoderNetwork = ContextEncoderNetwork(model=aModel, batch_size=batch_size, window_size=window_size,
                                               gap_length=gap_length, learning_rate=1e-4, name='nat_full_stft_3_')
