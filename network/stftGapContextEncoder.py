...
class StftGapContextEncoder(ContextEncoderNetwork):
    def __init__(self, model, batch_size, target_model, window_size, gap_length, learning_rate, name):
        ...
        self._SNR = tf.reduce_mean(self._pavlovs_SNR(self._target_model.output(), self._reconstructed_input_data,
                                                     onAxis=[1, 2, 3]))
    ...
    def reconstructAudio(self, audios, model_num=None, max_batchs=200):
        with tf.Session() as sess:
            ...
            reconstructed_stft = np.reshape(reconstructed, (-1, 11, 257, 2))
    ...
    def _reconstruct(self, sess, data_reader, max_steps):
        ...
        reconstructed = reconstructed.finalize()
        reconstructed = np.reshape(reconstructed, (-1, 11, 257, 2))
        out_gaps = out_gaps.finalize()
        out_gaps = np.reshape(out_gaps, (-1, 11, 257, 2))
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            try:
                ...
                for step in range(1, int(num_steps)):
                    ...
                    feed_dict = {self._model.input(): rec, self._target_model.input(): rec, self._model.isTraining(): True}
