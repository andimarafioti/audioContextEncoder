...
class ContextEncoderNetwork(object):
    ...
    def _network(self, dataset, isTraining):
        dataset = dataset - 0.5
        first_half = dataset[:, :(self._window_size - self._gap_length) // 2]
        second_half = dataset[:, (self._window_size - self._gap_length) // 2:]
        stacked = tf.stack([first_half, second_half], axis=2)
        model = SequentialModel(train_input_data=stacked, name="ContextEncoder")
        self._encoder(model, isTraining)

        model.addReshape((self._batch_size, 2048))
        model.addFullyConnectedLayer(2048, 2048, 'Fully', isTraining)
        model.addReshape((self._batch_size, 1, 32, 64))
    ...
    def _encoder(self, model, isTraining):
        with tf.variable_scope("Encoder"):
            model.addReshape((self._batch_size, 1, (self._window_size - self._gap_length)//2, 2))
            model.addConvLayer(filter_width=129, input_channels=2, output_channels=32,
                                      stride=(1, 1, 4, 1), name="First_Conv", isTraining=isTraining)
            model.addConvLayer(filter_width=65, input_channels=32, output_channels=64,
                                      stride=(1, 1, 4, 1), name="Second_Conv", isTraining=isTraining)
            model.addConvLayer(filter_width=17, input_channels=64, output_channels=64,
    ...
    def _decoder(self, model, isTraining):
        with tf.variable_scope("Decoder"):
            stride = 4
            signal_length = 32
            model.addDeconvLayer(filter_width=17, input_channels=64, output_channels=64,
                                 output_shape=[self._batch_size, 1, stride*signal_length, 64],
                                 stride=(1, 1, stride, 1), name="First_Deconv", isTraining=isTraining)
            signal_length *= stride
            model.addDeconvLayer(filter_width=65, input_channels=64, output_channels=16,
                                 output_shape=[self._batch_size, 1, stride*signal_length, 16],
                                 stride=(1, 1, stride, 1), name="Second_Deconv", isTraining=isTraining)
            signal_length *= stride
            stride = 2
            model.addDeconvLayer(filter_width=129, input_channels=16, output_channels=1,
                                 output_shape=[self._batch_size, 1, stride*signal_length, 1],
                                 stride=(1, 1, stride, 1), name="Last_Deconv", isTraining=isTraining)
